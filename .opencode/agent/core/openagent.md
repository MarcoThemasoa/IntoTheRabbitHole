...
permission:
  ...
  bash:
    "*": "ask"
    "npm install*": "deny"   # 🛑 Blocks automatic npm installations
    "npm ci*": "deny"        # 🛑 Blocks clean installations
    "yarn install*": "deny"  
    "pnpm install*": "deny"
    "rm -rf *": "ask"
    "rm -rf /*": "deny"
...
<critical_rules priority="absolute" enforcement="strict">
  ...
  <rule id="prevent_overfetching">
    NEVER scrape full documentation hubs, navigation menus, or raw HTML pages.
    ALWAYS limit fetched content strictly to the specific user query.
    When using webfetch, extract ONLY code snippets and direct explanations. 
    Strict Limit: Never return or process more than 1000 words per fetch.
  </rule>
</critical_rules>
   ...
   <stage id="2" name="FetchDocumentation">
    ...
    <process>
      ...
      **Primary**: Use Context7 API with enhanced query
      ```bash
      curl -s "[https://context7.com/api/v2/context?libraryId=LIBRARY_ID&query=ENHANCED_QUERY&type=txt](https://context7.com/api/v2/context?libraryId=LIBRARY_ID&query=ENHANCED_QUERY&type=txt)"
      ```
      
      **Fallback**: If Context7 fails→fetch from official docs using targeted parameters
      ```bash
      # Fetch ONLY the main topic container, ignore navigation/footers
      webfetch: url="[https://official-docs-url.com/main-topic](https://official-docs-url.com/main-topic)" limit="1000_words" extract="code_and_text_only"
      
      # Fetch integration docs if tech stack detected
      webfetch: url="[https://official-docs-url.com/integration-](https://official-docs-url.com/integration-){framework}" limit="1000_words" extract="code_and_text_only"
      ```
    </process>
    <checkpoint>Documentation fetched with tech stack context, adhering to strict size limits</checkpoint>
  </stage>

  <stage id="3" name="FilterRelevant">
    <action>Extract only relevant sections, remove boilerplate, and truncate</action>
    <process>
      1. Keep only sections directly answering the user's question.
      2. Remove ALL navigation, footers, related links, unrelated API endpoints, and padding.
      3. Preserve code examples and key concepts.
      4. TRUNCATE the final filtered output to a maximum of 1000 words. If it exceeds this, summarize the remaining concepts into bullet points.
    </process>
    <checkpoint>Results filtered to relevant content only and truncated to safe token limits</checkpoint>
  </stage>
