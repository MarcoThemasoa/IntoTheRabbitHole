...
export default function Home() {
  ...
  // 1. Add state to track if we are on a mobile device
  const [isMobile, setIsMobile] = useState(false);
  
  const observerRef = useRef<IntersectionObserver | null>(null);
  const heroSectionRef = useRef<HTMLElement>(null);

  // 2. Adjust the grid multiplier dynamically (less dense on mobile)
  const terminalGridMul = useMemo(() => {
    return isMobile ? [1.5, 1.5] as [number, number] : [2, 1] as [number, number];
  }, [isMobile]);

  useEffect(() => {
    // 3. Set up the resize listener to update isMobile
    const handleResize = () => {
        ...
        <div className="absolute inset-0 z-0">
          <Suspense fallback={<div className="absolute inset-0 bg-gradient-to-br from-blue-900 to-purple-900" />}>
            <FaultyTerminal
              ...
              //4. Dynamically increase scale and digit size for mobile 
              scale={isMobile ? 3 : 1.5}
              gridMul={terminalGridMul}
              digitSize={isMobile ? 2.5 : 1.2}
              timeScale={1}
              curvature={0.1}
              scanlineIntensity={0}
              glitchAmount={0.2}
